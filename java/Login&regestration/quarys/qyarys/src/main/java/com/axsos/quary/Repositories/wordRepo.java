package com.axsos.quary.Repositories;
...
public interface wordRepo extends CrudRepository<Country, Long> {
	List<Country> findAll();
	
	/*
	 * @Query(
	 * value="select A.name,B.language,B.percentage from countries AS A join languages AS B on A.id=B.country_id where B.language='Slovene' ORDER BY B.percentage DESC "
	 * , nativeQuery=true) List<Object[]> allCountries();*/
	
	 	 @Query(value="select A.name,B.language,B.percentage from countries AS A join language AS B on A.id=B.country_id where B.language='Slovene' ORDER BY B.percentage DESC ", nativeQuery = true)
