...
def show(request):
    ...
    if request.POST['hide']=='farm':
        ...
        request.session['gold']+=request.session['rand']
        if rand >= 0:
            request.session['activities'].append('Entered a casino and earned ' + str(rand) +' gold' + ' ' + '(' + str(datetime.now().strftime("%b-%d-%Y %H:%M-%p")) + ')')
        else:
            request.session['activities'].append('Entered a casino and lost ' + str(rand) + ' gold... Ouch...' + ' ' + '(' + str(datetime.now().strftime("%b-%d-%Y %H:%M-%p")) + ')')
